...
将 GPT-SoVITS 从一个需要手动配 YAML、手写路径的推理引擎，变成**开箱即用的 CLI 产品**。

| 特色 | 说明 |
|------|------|
| 🔍 **一键自动配置** | `--infer-auto` 扫描 `models/` 目录，自动发现权重文件、检测 GPU/CUDA、推断版本、生成正确路径。目前市面上唯一提供此功能的 GPT-SoVITS 工具 |
| 🎧 **流式合成 + 即时播放** | 边生成边播放，说完一句就开始出声。支持 4 种流式模式（分句 / 语义块 / 定长块 / 完整合成），无需等待全文合成完毕 |
| 🤖 **Claude Code 语音助手** | 唯一将 GPT-SoVITS 与 AI 编程助手深度集成的项目 —— Priestess 角色扮演、场景化 TTS 播报（权限申请、错误通知、进度报告、欢迎语） |
| 🧩 **独立工具链** | 不是 QQ/微信/nonebot 插件，不寄生在任何外部框架下。`install.sh` 一键安装，`tts-server` 管理服务生命周期 |
| 🎛️ **统一配置管理** | `tts-config` 一个工具管理所有配置（客户端 + 推理），交互式菜单 + CLI 参数双模式，不编辑任何 JSON 或 YAML |
| 🔄 **服务常驻 + 热切换** | 模型只加载一次，后续请求即时响应。支持运行时热切换 GPT/SoVITS 权重，无需重启服务 |
